...
/** 1) Este componente apenas carrega dados e decide o que mostrar.
 *    NÃO usa hooks depois de returns condicionais. */
function DashboardInner() {
  ...
  if (needsOnboarding) {
    return <Onboarding currencyDefault={user.currency} onSave={async (val, cur) => {
...
/** 2) Aqui ficam os hooks “depois” do carregamento: SEM returns condicionais antes deles. */
function DashboardContent({ user, trades, cashflows }:{
  user: UserDoc; trades: Trade[]; cashflows: Cashflow[];
}) {
  ...
  const tradesClosedMonth = useMemo(() => (
    trades
      .filter(t => t.status === "closed" && (t.closedAt! >= s.getTime() && t.closedAt! <= e.getTime()))
      .sort((a,b) => (a.closedAt! - b.closedAt!))
  ), [trades, s, e]);

  const monthSeries = useMemo(() => {
    let sum = 0; const arr = [{ t: s.getTime(), v: 0 }];
    for (const t of tradesClosedMonth) { sum += (t.pnl || 0); arr.push({ t: t.closedAt!, v: sum }); }
  ...
  let eq = user.startingBalance || 0, peak = eq, dd = 0;
  const daily = Array.from({ length: totalDays }, (_, i) => {
    ...
    if (eq > peak) peak = eq;
    dd = eq - peak;
    const pctCumul = user.startingBalance! > 0 ? ((eq - user.startingBalance!) / user.startingBalance!) * 100 : 0;
    return {
      key, date, pnl: dayPnL, trades: list.length, hasTrades, dd, pctCumul,
      isToday: ymd(new Date()) === key, openEq: eq - dayPnL, closeEq: eq, highEq: Math.max(eq, eq - dayPnL), lowEq: Math.min(eq, eq - dayPnL)
    };
  });

  const monthPnL = tradesClosedMonth.reduce((a, t) => a + (t.pnl || 0), 0);
  const monthPct = user.startingBalance! > 0 ? (monthPnL / user.startingBalance!) * 100 : 0;
  const monthWinRate = tradesClosedMonth.length ? (wins / tradesClosedMonth.length) * 100 : 0;
  ...
  const isCurrent = today.getFullYear() === viewYear && today.getMonth() === viewMonth;
  const daysLeft = isCurrent ? Math.max(0, Math.ceil((e.getTime() - today.getTime()) / 86400000)) : 0;

  const expenses = user.monthlyExpenses?.[`${viewYear}-${String(viewMonth+1).padStart(2,"0")}`] || 0;

  const growthPct = user.startingBalance! > 0 ? ((user.currentBalance! - user.startingBalance!) / user.startingBalance!) * 100 : 0;
  ...
  const months = Array.from({ length: 12 }, (_, m) => {
    const ms = startOfMonth(viewYear, m).getTime(), me = endOfMonth(viewYear, m).getTime();
    const list = trades.filter(t => t.status === "closed" && (t.closedAt! >= ms && t.closedAt! <= me));
    const pnl = list.reduce((a,t) => a + (t.pnl || 0), 0);
    const wr = list.length ? (list.filter(t => (t.pnl || 0) >= 0).length / list.length) * 100 : 0;
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="grid md:grid-cols-4 gap-3">
        <div className="card kpi"><span className="title">Saldo atual</span><span className="value">{fmtMoney(user.currentBalance || 0, user.currency)}</span></div>
        <div className="card kpi"><span className="title">% lucro desde início</span><span className={`value ${growthPct>=0?"text-ok":"text-danger"}`}>{isFinite(growthPct) ? growthPct.toFixed(2) : "—"}%</span></div>
        <div className="card kpi"><span className="title">P&L do mês</span><span className={`value ${monthPnL>=0?"text-ok":"text-danger"}`}>{fmtMoney(monthPnL, user.currency)}</span></div>
        <div className="card kpi"><span className="title">Trades (total)</span><span className="value">{trades.length}</span></div>
      </div>

      <div className="grid md:grid-cols-3 gap-3">
        <div className="card kpi"><span className="title">Máx. por trade (3%)</span><span className="value text-danger">{fmtMoney(maxPerTrade, user.currency)}</span></div>
        <div className="card kpi"><span className="title">Máx. no dia (9%)</span><span className="value text-danger">{fmtMoney(maxPerDay, user.currency)}</span></div>
        <div className="card kpi"><span className="title">Meta do dia (15%)</span><span className="value text-ok">{fmtMoney(dayGoal, user.currency)}</span></div>
      </div>
      ...
      {!annualMode ? (
        <CalendarMonth
          y={viewYear} m={viewMonth} daily={daily} currency={user.currency}
          onPrev={() => { let y=viewYear, m=viewMonth-1; if(m<0){m=11;y--;} setViewYear(y); setViewMonth(m); }}
          onNext={() => { let y=viewYear, m=viewMonth+1; if(m>11){m=0;y++;} setViewYear(y); setViewMonth(m); }}
      ...
      {!annualMode ? (
        <PayoutCard
          y={viewYear} m={viewMonth} currency={user.currency} monthPnL={monthPnL} daysLeft={daysLeft}
          monthsStats={{ expenses, sessionsSoFar: daily.filter(d => d.hasTrades).length, daysElapsed: daily.length, sessionRate: (daily.filter(d => d.hasTrades).length / daily.length) }}
        />
      ) : null}

      {/* Exports */}
      <ExportsCard
        year={viewYear} month={viewMonth} currency={user.currency}
        user={{ startingBalance: user.startingBalance || 0, currentBalance: user.currentBalance || 0 }}
        monthLabel={monthLabel(viewYear, viewMonth)}
        monthPnL={monthPnL} expenses={expenses}
        winRate={monthWinRate} daysLeft={daysLeft}
        daily={daily.map(d => ({ key: d.key, trades: d.trades, pnl: d.pnl, pctCumul: d.pctCumul, dd: d.dd, hasTrades: d.hasTrades }))}
...
function AnnualSwitch(props: any) {
  ...
  return <AnnualCard y={y} months={months} currency={currency} onBack={onBack} onPrevYear={onPrevYear} onNextYear={onNextYear} onPickYear={onPickYear} />;
}

/** Onboarding mantém-se igual */
function Onboarding({ currencyDefault, onSave }: { currencyDefault: string; onSave: (v: number, cur: string) => Promise<void> }) {
  ...
  const submit = async () => {
    ...
    if (!isFinite(v) || v <= 0) { setErr("Valor inválido"); return; }
  ...
  return (
    <div className="max-w-md mx-auto card">
      ...
      <div className="flex justify-end"><button className="btn" onClick={submit}>Guardar e continuar</button></div>
    </div>
  );
}
