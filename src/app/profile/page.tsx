...
import NextImage from "next/image"; // ⬅️ renomeado para não colidir com window.Image
...
async function compressImageToJpeg(file: File, maxEdge = 768, quality = 0.9): Promise<Blob> {
  ...
  const blob = await new Promise<Blob>((resolve, reject) =>
    canvas.toBlob((b) => (b ? resolve(b) : reject("toBlob")), "image/jpeg", quality)
  );
  return blob;
}
...
export default function ProfilePage() {
  ...
  const onPickFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      setBusy(true);
      // compressão leve para poupar storage/banda
      ...
      await uploadBytes(r, jpegBlob, { contentType: "image/jpeg", cacheControl: "public,max-age=31536000,immutable" });
      const publicUrl = await getDownloadURL(r);

      // atualiza o profile do auth (photoURL)
      if (auth.currentUser) {
        await updateProfile(auth.currentUser, { photoURL: publicUrl });
      }

      setMsg("Imagem atualizada com sucesso!");
      // substitui preview pelo URL final
      ...
      setBusy(false);
      // liberta o objectURL se era local
      try {
